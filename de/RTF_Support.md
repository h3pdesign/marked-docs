...
Wenn Sie die Datei RTF oder RTFD in einer anderen Anwendung speichern, erkennt Marked die Änderung und aktualisiert die Vorschau automatisch.

## Bilder

RTF definiert kein separates „Beschriftungsfeld“ in der Art und Weise, wie Cocoa nach HTML exportiert. Was in Ihrem Layout wie eine Überschrift aussieht, ist normalerweise **normaler Text** im nächsten Absatz, und Marked behält diesen als Fließtext bei.
...
- **Komplexes Layout** (mehrspaltige Tabellen, die nur zur Positionierung verwendet werden, Textfelder) werden möglicherweise nicht sauber auf Markdown abgebildet.
- **Gleichungen** in RTF werden in der Vorschau nicht unterstützt (siehe [MathJax](MathJax.html) für Markdown Mathematik)
- **Legacy `.doc`** (binäres Word) wird nicht unterstützt; Speichern Sie zuerst als DOCX oder RTF

Für ein einmaliges Einfügen ohne Speichern einer Datei verwenden Sie stattdessen [Clipboard Preview](Opening_Files.html#from-the-clipboard).

## Verwandte Themen

- [PDF Support](PDF_Support.html) – PDF-Dokumente als Markdown-Quellen öffnen
- [Working with DOCX](Working_with_DOCX.html) – Word-Dokumente mit Änderungsverfolgung und Kommentaren
- [Opening Files](Opening_Files.html) – Drag & Drop, Zuletzt geöffnet, Zwischenablage
- [Exporting](Exporting.html) – Kopieren Sie Rich Text und speichern Sie RTFD (Export), im Gegensatz zum Öffnen von RTF als Quelldatei