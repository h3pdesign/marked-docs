...
Nutzen Sie Ihre beiden Lieblings-Schreibgeräte gemeinsam.

> Marked kann immer noch Scrivener 2.0-Dateien lesen, aber die Entwicklung wird sich auf Version 3 nach Marked 2.5.11 konzentrieren.

## Scrivener 3.0 Grundlagen

Ziehen Sie ein Scrivener-Projekt (.scriv) nach Marked und es wird kompiliert und in der Vorschau angezeigt. Wenn Sie die Option zum Öffnen von .scriv-Dateien in Scrivener (oben) wählen, startet Marked auch Scrivener, wenn Sie die Datei nach Marked ziehen.
...
Wenn Sie ein Scrivener-Projekt in Marked öffnen, wird der Vorschauinhalt nur aus den von Ihnen ausgewählten Binderdokumenten erstellt. Für `.scriv`-Dateien ist die Filterung immer aktiv; Das Filterfeld ist lediglich eine praktische Möglichkeit, den Inhalt zu ändern.
...
Im Filterbereich werden die Ordnerbereiche Ihres Projekts aufgelistet (Entwurf, Forschung und andere Ordner der obersten Ebene außer Papierkorb). Jeder Abschnitt ist standardmäßig minimiert. Erweitern Sie einen Abschnitt, um seine Dokumente und Ordner in einer Gliederung anzuzeigen:
...
– Zeilen, für die **In Kompilierung einbeziehen** in Scrivener deaktiviert ist, werden ausgegraut angezeigt, Sie können sie jedoch weiterhin überprüfen, um eine Vorschau in Marked anzuzeigen.
...
Verwenden Sie **Alle auswählen** und **Alle abwählen** oben im Bedienfeld für das gesamte Projekt. Jede Abschnittsüberschrift verfügt nur für diesen Abschnitt über die Schaltflächen **Alle** und **Keine**. **Alle abwählen** bedeutet, dass keine Dokumente überprüft werden.

Wenn nichts ausgewählt ist, zeigt die Vorschau eine kurze Nachricht mit einem Link zum Öffnen des Filterpanels (`x-marked://scrivenerfilter`). Sie können diese URL in Skripten oder anderen Apps verwenden, um das Bedienfeld für das vordere Scrivener-Dokument in Marked aufzurufen.

Ihre Kontrollkästchenauswahlen werden pro Scrivener-Projekt gespeichert (durch die Projektkennung in der Datei `.scrivx`) und wiederhergestellt, wenn Sie das Projekt das nächste Mal in Marked öffnen. Wenn Sie ein Projekt zum ersten Mal öffnen, wählt Marked nur **Entwürfe**-Binderdokumente aus, deren Flag **In Kompilierung einbeziehen** auf Ja gesetzt ist (oder nicht gesetzt ist, was Scrivener als Ja behandelt). Recherchen und andere Ordner beginnen ungebremst; Von der Kompilierung ausgeschlossene Entwurfselemente beginnen ungeprüft, bleiben aber in der Liste auswählbar.
...
Marked kann für Sie auch hierarchische Markdown-Header basierend auf den Seiten Ihrer Scrivener-Datei erstellen. Um dies zu aktivieren, aktivieren Sie einfach die oben angezeigte Option.

## MultiMarkdown Metadaten

Wenn das erste Dokument in Ihrem Entwurfsordner den Namen „Metadaten“ trägt, wird es am Anfang des Vorschaudokuments als MultiMarkdown-Metadaten behandelt. Für diesen Abschnitt wird kein Header eingefügt, unabhängig von der Einstellung „Markdown Headers from Scrivener Titles“ (oben beschrieben), sodass der MultiMarkdown-Prozessor ihn als Metadaten lesen und entsprechende Ersetzungen und Exportoptionen zulassen kann.
...
Für externe (HTTP-)Links können Sie entweder die Syntax Markdown oder die Linkformatierung von Scrivener verwenden. Marked konvertiert vor der Verarbeitung das Scrivener-Format in Markdown.
...
Marked kann grundlegende Scrivener-Tabellen konvertieren. Wenn Sie jedoch eine Tabelle in Ihre Ausgabe einbinden möchten, tun Sie dies am besten in [MultiMarkdown table format](https://github.com/fletcher/MultiMarkdown/wiki/MultiMarkdown-Syntax-Guide#tables). (Eine App namens [TableFlip](http://tableflipapp.com/) kann das Generieren dieser Dateien zu einer einfachen Aufgabe machen.)
...
Bilder können in das Dokument Scrivener eingebettet oder mit der Bildsyntax Markdown referenziert werden. Die Markdown-Version eines Bild-Tags ist `![alt text](path/to/image.ext "optional title/description")`. Es kann auch das Referenzformat verwendet werden:
...
Relative Pfade, die auf dem übergeordneten Ordner des Dokuments basieren, funktionieren, und absolute Pfade ermöglichen den Zugriff auf Bilder überall, sind aber möglicherweise nicht so portierbar für die HTML-Ausgabe.
...
Unter [Additional app integrations](Additional_Application_Support.html) finden Sie eine Vorschau der Zwischenablage, Wiki-Links, Skripterstellung und die vollständige Liste der Handbücher pro App.
