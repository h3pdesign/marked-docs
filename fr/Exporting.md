...
## Le panneau d'exportation [tiroir]

![Panneau d'exportation][panneau d'exportation]
...
Utilisez la fonction Copier HTML pour placer le code source HTML de votre aperçu dans votre presse-papiers sans aucun problème. Vous pouvez le sélectionner dans le menu d'engrenage ou simplement appuyer sur {% kbd shift cmd C %}. Le code HTML copié sera un extrait prêt à être inséré dans un blog, un forum ou un document HTML.
...
![][exporthtmlaccessoire]



La commande Enregistrer HTML - accessible depuis le menu d'engrenage ou en tapant simplement **{% kbd cmd S %}** - vous permettra d'enregistrer un document complet prêt à être partagé ou publié.

Vous pouvez éventuellement inclure n'importe lequel des styles de Marked (ou l'un de vos [styles personnalisés][personnalisés]) dans votre exportation, vous donnant ainsi un document prêt à l'emploi avec le formatage nécessaire déjà intégré.
...
![Marges d'impression][marges d'impression]
...
`%title` utilisera n'importe quelle information « Titre : » définie dans les en-têtes de métadonnées MultiMarkdown, sinon elle reviendra au nom de fichier sans l'extension de fichier. Pour définir un titre à l'aide des métadonnées MMD, placez ce qui suit sur la première ligne du document :

    Titre : Le titre de votre document
...
Vous pouvez également spécifier une police d'en-tête/pied de page pour un document spécifique en utilisant les métadonnées MMD en haut du fichier :
    ...
    Police d’en-tête : Mensch

Notez que si vous utilisez un nom de famille de polices, une police par défaut sera sélectionnée. Vous pouvez spécifier une variante en utilisant le nom système de la police. Par exemple, dans le cas d'Helvetica Neue Ultralight, vous utiliserez « Police d'en-tête : HelveticaNeueUltralight ».

De plus, vous pouvez spécifier une taille de police d'en-tête/pied de page par document avec les métadonnées :

    Taille de la police d'en-tête : 10
...
Vous pouvez spécifier des en-têtes et des pieds de page pour chaque document à l'aide des métadonnées MultiMarkdown tout en haut du document :
    ...
    Imprimer l'en-tête gauche : %title
    Centre d'en-tête d'impression : %basename
    Imprimer l'en-tête à droite : %date
    Imprimer le pied de page à droite : %page/%total
...
Marked peut exporter des données RTF (Rich Text Format) directement dans votre presse-papiers. Choisissez simplement la commande Copier le texte enrichi dans le menu d'engrenage.
...
Avant de télécharger, vous souhaiterez peut-être vérifier à quoi ressemblera votre EPUB sur les appareils Kindle à l'aide du [Kindle Previewer] gratuit d'Amazon (https://kdp.amazon.com/help/topic/G202131170). Il s'agit d'un logiciel tiers facultatif d'Amazon, qui ne fait pas partie de Marked.
...
- De là, vous pouvez :
  ...
  - **Dupliquez** un profil pour en créer un nouveau basé sur celui-ci
...
- **Math Rendering** : intégration MathJax/KaTeX et numérotation des équations
...
Les profils sont stockés dans votre dossier Application Support à l'adresse :
    ...
    ~/Bibliothèque/Application Support/Marked/ExportProfiles.plist
