...
- _fileIncludes_ teste si le fichier contient des éléments inclus
  fichiers (en utilisant l'un des [Marked's include
  syntaxes](Multi-File_Documents.html)).
- _metaType_ teste si le fichier inclut YAML,
  Métadonnées MultiMarkdown ou Pandoc
- _metadata.X_ teste des clés de métadonnées spécifiques comme l'auteur,
  date, titre, etc.

Pour faire correspondre tous les fichiers (c'est-à-dire un processeur personnalisé qui toujours
courses), réglez `filename` sur `contains` `*`. L'astérisque sera
...
**MARKED_INCLUDES**
: Une liste entre guillemets, séparés par des virgules, des fichiers marqués inclus dans le texte transmis à l'aide des différentes [syntaxes d'inclusion](Special_Syntax.html#pagebreaks).
