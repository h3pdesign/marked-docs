...
L'un des avantages de l'utilisation de fichiers CSV au lieu d'écrire des tableaux Markdown est que les sauts de ligne dans les cellules sont automatiquement convertis en balises `<br>`. This has to be done manually to include line breaks when writing MultiMarkdown tables, so it's an additional time-saver.
