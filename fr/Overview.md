...
{% apponly div %}
*Pour la dernière version de cette documentation, visitez la [version en ligne][aide].*
{% endapponly %}

**Assurez-vous de consulter la collection croissante de [Vidéos tutorielles marquées](https://www.youtube.com/playlist?list=PLNoOPRHOBV0ptk_ka386xltm3VyVw6QgZ).**
...
Marked fonctionne avec GitHub Flavored Markdown, CommonMark (GFM), Kramdown et MultiMarkdown, et peut convertir la syntaxe de plusieurs variantes pour un aperçu (il peut également être étendu pour fonctionner avec à peu près n'importe quel processeur dont vous avez besoin, y compris Textile, reStructuredText, Wikitext et plus). Je suppose que --- puisque vous êtes ici --- vous savez ce qu'est au moins un de ces langages de balisage. Sinon, vous devriez commencer par [Markdown Basics] [daringfireball] de John Gruber, consultez [MarkdownGuide.org] [mdguide]. Marked comprend un guide de syntaxe Markdown complet dans le menu d'aide.
...
* Vous pouvez également utiliser Marked sans éditeur. Faites simplement glisser les fichiers Markdown vers l'icône pour les prévisualiser, les imprimer et les exporter au format source PDF, DOC, RTF ou HTML. Marqué peut également **ouvrir les fichiers `.rtf` et `.rtfd`** en tant que documents sources ([Support RTF et RTFD](RTF_Support.html)).
