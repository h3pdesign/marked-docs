...
![URL Markdownify][importurl]
  ...
  [importurl] : import-url@2x.jpg largeur = 800 classe = centre
...
3. **Cliquez sur "Automatique"** ou appuyez sur `Return` pour extraire le contenu
4. Le contenu extrait s'ouvrira **automatiquement** dans un nouveau document marqué
...
- Vous devez toujours cliquer sur **"Automatique"** ou appuyer sur `Return` pour le traiter
...
- **Copiez le contenu extrait** et collez-le dans les documents marqués existants

### Intégration avec Marked

Le contenu extrait s'ouvre dans Marqué avec :

- **Nom automatique des fichiers** basé sur le titre de l'article
- **Préservation de l'URL source** dans les métadonnées du document
- **Capacités complètes marquées** pour la lecture et l'exportation)
