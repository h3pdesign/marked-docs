...
- **Wrapper Padding** : espace autour de la zone de contenu
- **Retrait du paragraphe** : retrait de la première ligne des paragraphes
- **List Indent** : Indentation pour les listes
- **Blockquote Indent** : Marge gauche pour les blockquotes
...
- **Body Color** : couleur du corps du texte
- **Link Color** : Couleur des liens
- **Couleur d'accentuation** : Couleur du texte souligné (`<em>`)
- **Strong Color** : Couleur pour un texte fort (`<strong>`)
- **Marquer l'arrière-plan** : Couleur d'arrière-plan pour le texte en surbrillance (`<mark>`)
...
Activez le **Mode sombre** pour prévisualiser et configurer les couleurs du mode sombre. Lorsqu'il est activé, vous verrez des commandes de couleur distinctes pour les variantes du mode sombre. Les styles du mode sombre s'appliquent lorsque `.inverted` est défini sur l'élément body dans Marqué.
...
- **Border Color** : Couleur de bordure pour les cellules
...
- **Cell Padding** : espacement interne
...
- **Rayon de bordure** : coins arrondis
- **Mode Wrap** : Pas d'emballage (pré-emballage), Conserver + emballage (pré-emballage) ou Normal (emballage)
- **Code Padding** : espacement interne
...
- **Marker Color** : Couleur des marqueurs de notes de bas de page
...
Cliquez sur **Ajouter à Marked** pour ajouter directement le style à votre installation marquée. Cela nécessite que Marked soit en cours d'exécution et ouvrira une boîte de dialogue pour confirmer le nom du style et les informations sur l'auteur.

### Partager le style

Cliquez sur **Partager le style** pour publier votre style dans la [Galerie de styles marqués](https://markedapp.com/styles) pour que d'autres puissent l'utiliser. Vous devrez fournir :
