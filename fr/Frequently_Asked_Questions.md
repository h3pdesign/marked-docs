...
Si vous effectuez une nouvelle installation de Marked 2 et recevez l'erreur « Le code de licence a déjà été utilisé » lors de la saisie de votre licence, veuillez <a href="mailto:me@brettterpstra.com">contactez-moi</a> pour demander une nouvelle licence. **Veuillez inclure l'adresse e-mail sous laquelle vous vous êtes inscrit et/ou votre code de licence actuel.**

Les premières licences générées pour Marked avaient une limite d'utilisation au lieu d'une limite de machine, donc 3 installations --- même sur la même machine --- utiliseraient des activations. Ces limites ont été corrigées dans les licences générées plus récemment. L'achat d'une licence Marked 2 vous permet d'installer Marked 2 sur n'importe quelle machine que vous possédez, alors n'hésitez pas à demander un remplacement si vous rencontrez des problèmes.
...
1. Assurez-vous d'être allé dans les préférences **Marquées 2** (⌘,), dans le volet **Avancé** et d'avoir cliqué sur « Mettre à jour les autorisations ». Cela tentera d'accorder un accès marqué à l'intégralité de votre lecteur par défaut, résolvant ainsi les problèmes liés aux scripts et aux utilitaires qui doivent accéder aux dossiers temporaires et aux emplacements non par défaut.
2. Essayez d'utiliser un script. Référencez l'utilitaire que vous souhaitez exécuter (multimarkdown, kramdown, etc.) dans un script shell. Il peut s'agir d'un script bash, Ruby, Perl ou Python. Définissez ensuite le processeur dans les préférences avancées sur le shell ou l'exécutable associé, et les paramètres sur l'emplacement du script. Par exemple, je peux créer un script bash enregistré dans ~/scripts/mmd_wrapper.sh :
        ...
        #!/bin/bash
        chat | /usr/local/bin/multimarkdown
    
    Rendez-le ensuite exécutable (`chmod a+x ~/scripts/mmd_wrapper.sh`) et configurez mes préférences de processeur personnalisé :
...
## Chemins relatifs dans les fichiers inclus [relative_paths_in_inclus_files]
...
Si `included_file.md` fait référence à image1.jpg via un chemin relatif, il doit être écrit comme `../images/image1.jpg`, _not_ `images/image1.jpg`. (`..` indique le répertoire parent) .
...
Si vous préférez un remboursement, il vous suffit de <a href="mailto:me@brettterpstra.com">envoyez-moi un e-mail directement</a> avec le compte de messagerie utilisé pour la licence ou l'ID de transaction figurant sur le reçu.
