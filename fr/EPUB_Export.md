...
Ouvrez le [Panneau d'exportation](Exporting.html#drawer) ({% kbd shift cmd e %}) ou utilisez **Enregistrer sous** dans le menu d'engrenage et choisissez **EPUB**.
...
* **Image de couverture** --- PNG ou JPG facultatif ; Marqué génère un aperçu de la couverture par défaut lorsqu'aucun n'est défini
...
Le **style d'aperçu** sélectionné pour votre document détermine l'apparence de l'EPUB. Chaque thème marqué intégré --- Swiss, GitHub, Manuscript et le reste --- peut être appliqué à l'exportation EPUB.
...
3. Exporter vers EPUB --- Marqué regroupe le CSS du thème dans le package EPUB.
...
Les lecteurs I> EPUB ignorent certains CSS réservés au Web (positionnement fixe, astuces de fenêtre d'affichage, etc.). Ce que vous voyez dans l'aperçu de Marked est l'objectif, mais les moteurs de mise en page des liseuses peuvent simplifier l'espacement et les polices. Testez dans Apple Books ou votre lecteur cible avant de publier.
...
* Marqué intègre votre feuille de style dans l'EPUB exporté.
...
* [Mode sombre](Previewing.html) l'inversion dans l'aperçu utilise `@media screen` requêtes ; la plupart des lecteurs EPUB utilisent la feuille de style claire, sauf si l'application de lecture applique son propre thème sombre.
...
Pour obtenir des conseils sur la création, voir [Création de CSS personnalisé](Writing_Custom_CSS.html).
...
Pour les travaux longs, compilez des chapitres avec [Documents multi-fichiers](Multi-File_Documents.html) --- fichiers d'index, exportations Scrivener, Leanpub `Book.txt` ou index de style GitBook. Les montres marquées incluent les fichiers, prévisualisent le livre complet et exportent un EPUB à partir du HTML compilé.

Les titres du document compilé deviennent l'EPUB [table des matières](Document_Navigation.html) pour la navigation dans Apple Books et d'autres lecteurs.
...
Faites glisser un `.epub` exporté sur l'application Livres ou ajoutez-le via **Fichier → Importer**. La typographie personnalisée et la pochette de votre thème marqué sont conservées. Utilisez Apple Books sur Mac, iPad ou iPhone pour vérifier la mise en page avant de partager.
...
**MOBI n'est pas requis** pour les nouveaux titres KDP. Marqué n’exporte pas MOBI.
