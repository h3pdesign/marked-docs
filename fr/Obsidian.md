...
L'aperçu intégré d'Obsidian est idéal lorsque vous ne quittez jamais l'application. Choisissez Marqué lorsque vous souhaitez une exportation de qualité publication, une relecture avancée, des thèmes CSS personnalisés ou le même flux de travail de prévisualisation en direct sur plusieurs éditeurs. Voir [Marked vs Obsidian Preview](Marked_vs_Obsidian_Preview.html) pour une comparaison complète.

## Ouvrir un coffre-fort entier

Faites glisser le **dossier du coffre-fort** (le répertoire qui contient le dossier de configuration caché d'Obsidian à la racine du coffre-fort) sur Marqué dans le Dock. Marqué surveille cet arbre, conserve la note **la plus récemment modifiée** dans l'aperçu et se met à jour au fur et à mesure que vous enregistrez dans Obsidian.
...
## Syntaxe des légendes d'obsidienne

Lorsque le processeur MultiMarkdown s'exécute, Marked convertit les **légendes courantes de style Obsidienne** (le motif `> [!note]`) en balisage de bloc stylisé afin qu'elles correspondent au reste de votre aperçu.

## Marqué 3 Plugin Obsidienne

Le [plugin Marked 3 Obsidian][plugin] peut ouvrir la note actuelle ou l'ensemble du coffre-fort avec des commandes ou des raccourcis clavier afin que la fenêtre Marked suive ce que vous modifiez. Utilisez la palette de commandes (**⌘P**) et recherchez **Marqué**, ou attribuez des raccourcis clavier dans les paramètres **Hotkeys** d'Obsidian.
...
4. Dans Obsidian, ouvrez **Paramètres → Plugins communautaires**, désactivez le **Mode restreint** si nécessaire et activez **Ouvrir dans marqué**.
